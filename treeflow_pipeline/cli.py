...
def get_vi_config(ctx, optimizer, learning_rate, num_steps, rescaling, sample_size):
    return dict(
        seed=ctx.obj.seed,
        optimizer=optimizer,
        num_steps=num_steps,
...
@click.option("--sample-size", type=int, default=1)
...
def variational_fit(
    ...
    res = treeflow_pipeline.model.get_variational_fit(
        ...
        start_values_dict,
        ctx.obj.model,
        get_vi_config(ctx, optimizer, learning_rate, num_steps, rescaling, sample_size),
        clock_approx,
    )
